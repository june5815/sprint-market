...
import { attachIsLiked } from '../lib/isLikedUtil.js';

export async function createArticle(req, res) {
  const userId = req.user?.userId;
  if (!userId) {
    return res.status(401).send({ message: '로그인된 사용자만 게시글을 등록할 수 있습니다.' });
  }
...
export async function updateArticle(req, res) {
  ...
  if (article.userId !== userId) {
    return res.status(403).send({ message: '게시글을 등록한 사용자만 수정할 수 있습니다.' });
  }
...
export async function deleteArticle(req, res) {
  ...
  if (existingArticle.userId !== userId) {
    return res.status(403).send({ message: '게시글을 등록한 사용자만 삭제할 수 있습니다.' });
  }
...
export async function createComment(req, res) {
  const userId = req.user?.userId;
  if (!userId) {
    return res.status(401).send({ message: '로그인된 사용자만 댓글을 등록할 수 있습니다.' });
  }
